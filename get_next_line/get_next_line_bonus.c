...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/09/27 13:01:07 by atopalli          #+#    #+#             */
/*   Updated: 2022/09/28 08:35:10 by atopalli         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_gnl(t_struct *gnl, int fd)
{
	...
	{
		gnl->temp[gnl->buf] = 0;
		str[fd] = ft_strcpy(str[fd], gnl->temp, &gnl->flag);
		if (gnl->flag == 0)
			gnl->buf = read(fd, gnl->temp, BUFFER_SIZE);
		else
			break ;
	...
	if (str[fd])
	{
		gnl->temp = ft_retline(str[fd]);
		str[fd] = ft_restfile(str[fd], ftstrlen(gnl->temp));
		if (gnl->temp)
			return (gnl->temp);
	}
	return (0);
